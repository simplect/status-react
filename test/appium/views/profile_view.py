...
class ProfileView(BaseView):
    ...
    def edit_profile_picture(self):
        if AbstractTestCase().environment == 'sauce':
            ...
            self.element_by_text('sauce_logo.png').click()
            self.confirm_button.click()
        else:
            raise NotImplementedError('Test case is implemented to run on SauceLabs only')

    def verify_profile_picture(self):
        template = os.sep.join(__file__.split(os.sep)[:-2]) + '/tests/templates/profile_picture.png'
        self.profile_picture.find_element().screenshot('current_picture.png')
        current_image = os.sep.join(__file__.split(os.sep)[:-2]) + '/tests/current_picture.png'
        if ImageChops.difference(Image.open(current_image, 'r'), Image.open(template, 'r')).getbbox():
            pytest.fail('Profile picture is not updated')
